...
public class UsersSystemTests extends AbstractSysTest {
    ...
    void updateUser() {
        List<User> createdUsersList = TestFixtures.createUsers(userService);
        User userToUpdate = createdUsersList.getFirst();

        // update fields using toBuilder() pattern (records are immutable)
        userToUpdate = userToUpdate.toBuilder()
                .loginName(userToUpdate.loginName() + "u")
                .emailAddress(userToUpdate.emailAddress() + "u")
                .firstName(userToUpdate.firstName() + "u")
                .lastName(userToUpdate.lastName() + "u")
                .build();
